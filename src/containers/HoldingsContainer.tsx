...
const initializeHolding = (
  ...
  const usdPrice = currency === Currency.USD ? sharePrice : (sharePrice / exchangeRate);
  const shareDiff = Math.floor((netWorth * targetPercent / usdPrice) / 100 - shareCount);
  return {
      cadValue,
      currency,
      currentPercentage,
...
export function mapDispatchToProps(dispatch: Dispatch<actions.ActionType>) {
  return {
    ...
    onUpdateTarget: (ticker: string, targetPercent: number) => dispatch(actions.updateTargetPercent(ticker, targetPercent)),
  };
}
